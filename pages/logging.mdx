...
$logger->setMaxFileSize(10485760)  // 10MB
    ->setMaxFiles(5)
    ->setRotationInterval('daily');
```
...
$logger->setLogHandler(new ElasticsearchHandler([
...
$logger->setLogFilter(function($data) {
...
$logger->setAlertHandler(function($level, $message, $context) {
    if ($level === 'error') {
